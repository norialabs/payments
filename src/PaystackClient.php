...
class PaystackClient
{
    ...
    public static function make(
        ...
        $transport = new HttpTransport(
            http: $httpFactory,
            baseUrl: Setting::string($config['base_url'] ?? null, self::BASE_URL),
    ...
    private function resolveEndpoint(string $name, array $replacements = []): array
    {
        ...
        foreach ($replacements as $key => $value) {
            $path = str_replace('{'.$key.'}', rawurlencode(Setting::string($value)), $path);
        }
    ...
    private static function resolveEndpoints(array $config): array
    {
        ...
        foreach ((array) ($config['endpoints'] ?? []) as $name => $override) {
            ...
            if (is_array($override)) {
                ...
                if ($path !== null && $path !== '') {
                    $endpoints[$name] = [strtoupper(Setting::string($method)), Setting::string($path)];
                }
            }
        }
